...
function drawMap() {
    ...
    console.log('绘制地图:', { 
        ...
        view: { centerX: currentView.centerX, centerY: currentView.centerY, zoom: currentView.zoom }
    });
...
function drawBoundaries() {
    ...
    console.log(`绘制 ${boundaries.length} 个边界区域 (currentCity: ${currentCity})`);
    ...
    console.log(`边界绘制完成: 绘制了${drawnCount}个，跳过了${skippedCount}个`);
}
...
async function loadData() {
    try {
        ...
        if (window.normalizedData && window.normalizedData.events && window.normalizedData.events.length > 0) {
            // 使用标准化数据中的事件
            events = window.normalizedData.events;
            console.log('=== map-canvas 事件数据加载 ===');
            console.log('数据源: normalizedData.events');
...
function onCanvasClick(e) {
    ...
    if (currentCity && stats.events && stats.events.length > 0) {
        const eventList = stats.events.slice(0, 10).map(evt => 
            evt.properties?.名称 || evt.properties?.事件名称 || evt.properties?.name || '未知事件'
        ).join('\n');
...
function updateBreadcrumb() {
    ...
    if (currentCity) {
        const city = WANBEI_CITIES.find(c => c.key === currentCity);
        breadcrumbEl.textContent = city ? city.fullName : currentCity;
    } else {
        breadcrumbEl.textContent = '六市总览';
    }
}

// 更新地图区域标题
function updateMapSectionTitle() {
    const titleEl = document.getElementById('mapSectionTitle');
    if (!titleEl) return;
    
    if (currentCity) {
        const city = WANBEI_CITIES.find(c => c.key === currentCity);
        titleEl.textContent = `分布结果 - ${city ? city.fullName : currentCity}`;
    } else {
        titleEl.textContent = '分布结果';
    }
}
