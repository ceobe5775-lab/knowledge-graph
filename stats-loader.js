    ...
    try {
        ...
        if (data.summary) {
            stats = {
                ...
                persons: data.summary.persons || 0,
                locations: data.summary.locations || 0,
                times: data.summary.times || 0,
            ...
            stats = {
                events: data.combined.summary.events || 0,
                persons: data.combined.summary.persons || 0,
                locations: data.combined.summary.locations || 0,
                times: data.combined.summary.times || 0,
                total_nodes: data.combined.summary.total_nodes || 0,
                total_relationships: data.combined.summary.total_relationships || 0
            };
        }
        
        // 如果summary中没有数据，则从数组长度计算
        if (!stats || (stats.events === 0 && stats.persons === 0 && stats.locations === 0)) {
            const combined = data.combined || data;
            stats = {
                events: combined.events?.length || 0,
                persons: combined.persons?.length || 0,
                locations: combined.locations?.length || 0,
                times: combined.times?.length || 0,
                total_nodes: combined.nodes?.length || 0,
                total_relationships: combined.relationships?.length || 0
            };
            
            // 如果仍然没有数据，尝试从nodes中筛选
            if (stats.events === 0 && combined.nodes) {
                stats.events = combined.nodes.filter(n => 
                    n.labels && n.labels.some(l => l.includes('事件'))
                ).length;
            }
            
            if (stats.persons === 0 && combined.nodes) {
                stats.persons = combined.nodes.filter(n => 
                    n.labels && n.labels.some(l => l.includes('人物'))
                ).length;
            }
            
            if (stats.locations === 0 && combined.nodes) {
                stats.locations = combined.nodes.filter(n => 
